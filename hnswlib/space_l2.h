...
static dist_t L2SqrAtLeast(const data_t *__restrict pVect1,
                           const data_t *__restrict pVect2, const size_t qty) {
  return L2Sqr<dist_t, data_t, K>(pVect1, pVect2, K) +
         L2Sqr<dist_t, data_t, 1>(pVect1 + K, pVect2 + K, qty-K);
}
...
static float L2SqrSIMD16ExtResiduals(const float *pVect1, const float *pVect2,
  ...
  float res_tail = L2Sqr<float>(pVect1Offset, pVect2Offset, qty);
  return (res + res_tail);
}
#endif
...
static float L2SqrSIMD4ExtResiduals(const float *pVect1, const float *pVect2,
  ...
  float res_tail = L2Sqr<float>(pVect1 + qty4, pVect2 + qty4, qty_left);
...
class L2Space : public SpaceInterface<dist_t, data_t> {
...
template <>
L2Space<float, float>::L2Space(size_t dim)
